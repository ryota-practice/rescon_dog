...
void servo_samemove(int* ang, int* offset, bool* servosw, int resolution, int time,
                    double x1, double y1, double z1, double x2, double y2, double z2) {  //start,goal
  double step_x = (x2 - x1) / resolution;
  double step_y = (y2 - y1) / resolution;
  double step_z = (z2 - z1) / resolution;

  for (int i = 0; i <= resolution - 1; i++) {
    // Serial.println(i);
    pos2ang(ang, FR, x1 + step_x * i, y1 + step_y * i, z1 + step_z * i);
    pos2ang(ang, FL, x1 + step_x * i, y1 + step_y * i, z1 + step_z * i);
    pos2ang(ang, RR, x1 + step_x * i, y1 + step_y * i, z1 + step_z * i);
    pos2ang(ang, RL, x1 + step_x * i, y1 + step_y * i, z1 + step_z * i);
    servo_all(ang, offset, servosw);
    delay(time);
  }
}
